...
- **Islamic Umm al-Qura** (`islamic-umalqura`) is Saudi Arabia's official Hijri calendar, using observation-based month lengths from KACST. Uses a 301-entry baked data table (`PackedHijriYearData`, UInt16 per year) for 1300–1600 AH (~1882–2174 CE), falling back to Islamic Civil arithmetic outside that range. Month lengths from ICU4X/ICU4C; offsets recomputed for our epoch (R.D. 227015) — ICU4X uses R.D. 227016 for the same Julian date due to a proleptic Julian formula difference. See `Docs/Islamic.md`.
