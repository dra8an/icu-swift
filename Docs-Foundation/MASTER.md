...
| **`FOUNDATION_APPLE.md`** | Orientation for Apple's **internal** Foundation-rizz repo (vs. the public `swift-foundation` mirror that `01-FoundationCalendarSurface.md` references). Calendar file inventory with line counts, the ~3,475-line "comes for free" surface size, swift-foundation issue #1842 (`ucal` overhead — internal validation of our pitch), empirical `ucal_clear` findings, build/test workflow, PR conventions, CI capsules. |
