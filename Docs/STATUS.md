...
**Calendars: 23 implemented (all at 100% accuracy except Chinese 3/2461 known limitation). 319 tests. Formatting: not started.**
...
| Islamic Umm al-Qura | `islamic-umalqura` | Baked data | 301-entry KACST table (1300–1600 AH) |
...
Chinese calendar uses `PackedChineseYearData` (UInt32) stored in `ChineseDateInner` — all field access is lock-free table lookup for 1901–2099. Moshier fallback via `ChineseYearCache` (LRU) outside baked range.

See `Docs/CalendarAstronomical.md`, `Docs/Islamic.md`, `Docs/Chinese.md`.
...
See `Docs/TestCoverageAndDocs.md` for the full per-calendar regression index.

## What's Not Done

See `Docs/NEXT.md` for prioritized next steps.

Formatting/parsing infrastructure not started (Phase 8–10).
