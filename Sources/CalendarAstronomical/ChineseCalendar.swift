...
struct ChineseYearData {
    ...
    static func compute<V: EastAsianVariant>(relatedIso: Int32, variant: V.Type) -> ChineseYearData {
        ...
        // Use Reingold's estimatePriorSolarLongitude for the initial estimate,
        // then refine using the engine's solarLongitude.
        let searchMoment = midnight(RataDie(jan1.dayNumber + 30))
        let solsticeMoment = Astronomical.estimatePriorSolarLongitude(
            angle: 270.0,
            moment: searchMoment
        )

        // Refine: find the day where solar longitude first exceeds 270°
        ...
        // Find the new moon on or after the solstice (month 11 of previous year)
        let nm11 = newMoonOnOrAfter(solsticeRd)
        ...
        let nextSolsticeMoment = Astronomical.estimatePriorSolarLongitude(
            angle: 270.0,
            moment: nextSearchMoment
        )
        ...
        // Count new moons between solstices
        let moonsBetweenSolstices = newMoons.filter { $0 >= nm11 && $0 < newMoonOnOrAfter(nextSolsticeRd) }.count
        let hasLeapMonth = moonsBetweenSolstices > 12

        // Compute major solar terms at each new moon
        let majorSolarTerms: [UInt32] = newMoons.map { majorSolarTerm($0) }

        // Find the new year (second new moon after the winter solstice)
        // Following ICU4X's new_year_in_sui: check if month 11 or 12 is a leap month
        // (i.e., has no zhōngqì). A month has no zhōngqì when its solar term equals
        // the NEXT month's solar term (forward comparison).
        var newYearIndex = 1
        if hasLeapMonth {
            newYearIndex = 1
            // Check if month 11 or 12 (indices 0 or 1) is a leap month
            for i in 0..<min(2, newMoons.count - 1) {
                if i + 1 < majorSolarTerms.count
                   && majorSolarTerms[i] == majorSolarTerms[i + 1] {
                    // Month at index i has no zhōngqì — it's a leap month before new year
                    newYearIndex = i + 2
                    break
                }
            }
            if newYearIndex > 2 { newYearIndex = 2 }
        }

        let newYear = newMoons[newYearIndex]

        // Build month lengths and find leap month.
        // Following ICU4X's get_leap_month_from_new_year: the leap month is the first
        // month whose solar term equals the NEXT month's solar term (forward comparison).
        // This means the current month contains no zhōngqì (no 30° crossing during it).
        ...
        for i in newYearIndex..<min(newYearIndex + 13, newMoons.count - 1) {
            ...
            if hasLeapMonth && leapMonthNum == nil {
                // Forward comparison: does THIS month's term equal the NEXT month's term?
                // If so, this month has no zhōngqì and is the leap month.
                if i + 1 < majorSolarTerms.count
                   && majorSolarTerms[i] == majorSolarTerms[i + 1] {
                    leapMonthNum = monthNum - 1
                    monthLengths.append(isLong)
                    continue  // Don't increment monthNum — leap month duplicates preceding month number
                }
            }
            ...
            if monthNum > 12 && leapMonthNum == nil && !hasLeapMonth {
                break
            }
            if monthNum > 13 {
                break
        ...
        while monthLengths.count < 12 {
            monthLengths.append(false)  // 29-day fallback
        }

        // Trim to 12 or 13 months
        if !hasLeapMonth && monthLengths.count > 12 {
            monthLengths = Array(monthLengths.prefix(12))
        }
        if monthLengths.count > 13 {
            monthLengths = Array(monthLengths.prefix(13))
        }
